...
import { countWords, type AgentDocument } from "@agent-context-bridge/shared";
...
chrome.runtime.onMessage.addListener((message: ExtractPageMessage, _sender, sendResponse) => {
  ...
  try {
    sendResponse({ ok: true, document: extractReadableDocument() } satisfies ExtractPageResponse);
  } catch (error) {
    sendResponse({ ok: false, error: error instanceof Error ? error.message : "Unknown extraction error" } satisfies ExtractPageResponse);
  }
